...
- **Copiloto de IA Multiagente (Zero Downtime)** com esteira de resiliência em camadas: **SambaNova (Meta-Llama 3.3 70B / 3.1 8B, gratuito)** ➔ **OpenRouter (DeepSeek V3)** ➔ **OpenRouter (Claude 3.5 Haiku, pago)** — com **economia de tokens**, logs de consumo de LLM e **motor determinístico de fallback** por regras. Além disso, executa **automação ativa via MCP Local** (in-process, custo zero): abrir/mover chamados no Kanban, consultar a Base de Conhecimento (RAG) e **gerar parecer de segurança com modelagem STRIDE em Markdown (.md)** com download direto no chat (5 ferramentas) — detalhes em `docs/architecture/MCP_ARCHITECTURE.md` e `docs/functional/MCP_FUNCTIONAL_SPEC.md`.
...
- **Capacidades**:
  ...
  - Roteamento transparente em cascata: no **chat**, **SambaNova** (`Meta-Llama-3.3-70B-Instruct` → `Meta-Llama-3.1-8B-Instruct`) ➔ **OpenRouter** (`deepseek/deepseek-chat` → `claude-3-5-haiku`, pagos); no **Security QA**, Google Gemini, OpenAI, OpenRouter e Groq (ambos com limite de 6s via `AbortSignal.timeout(6000)` e `maxRetries: 0` para evitar estouros de tempo limite na Vercel).
...
- **Multiagent AI Copilot (Zero Downtime)**: Layered resilience pipeline — **SambaNova (Meta-Llama 3.3 70B-Instruct, free)** → **SambaNova (Meta-Llama 3.1 8B-Instruct, free)** → **OpenRouter (DeepSeek V3, paid)** → **OpenRouter (Claude 3.5 Haiku, paid)** — with token-economy directives, deterministic rule-based fallback, strict Cybersecurity persona, RAG over the 314 SD v4.1 requirements (static JSON + **pgvector semantic retrieval** over `knowledge_articles`), and automatic memory hygiene on login. Every call is persisted to `llm_call_logs` (provider, model, latency, tokens, estimated cost) for the LLM Consumption Panel. It also enables **active automation via local MCP** (in-process, zero cost): create/move Kanban tickets, query the Knowledge Base (RAG), and **generate a STRIDE security assessment in Markdown (.md)** downloadable directly in the chat (5 tools).
