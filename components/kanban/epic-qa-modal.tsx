...
const PHASE_LABELS: Record<string, string> = {
  ...
  analysis: "Análise via motores de IA",
...
export function EpicQaModal({ ticket, sprints, onClose }: EpicQaModalProps) {
  ...
  const [streamState, setStreamState] = useState<{ phase: string | null; message: string | null; partial: Partial<QaAnalysis> | null }>({
    phase: null,
    message: null,
    partial: null,
  });
  ...
  }, [evidence, environmentUrl, requirements, ticket.title, router]);
  ...
  const handlePreloadRequirements = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Falha ao pré-carregar requisitos.");
    }
  }, [ticket]);
      ...
      <div className="relative bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden animate-slideUp">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)] space-y-5">
          ...
          <div className="flex items-center justify-between pt-2">
            <p className="text-xs text-gray-500">
              {canRun
                ? "Pronto para iniciar a análise do épico."
                : running
            ...
            <Button onClick={handleRun} disabled={!canRun} className="gap-2">
              ...
              {running ? "Analisando..." : "Executar Teste"}
            </Button>
          </div>
          ...
          {(running || streamState.partial || resultId || error) && (
            <div className="rounded-lg border border-gray-200 bg-slate-50 p-4 space-y-4">
              ...
              {streamState.partial && (
                <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-3 animate-fadeIn">
              ...
              {resultId && (
                <div className="rounded-lg border border-green-200 bg-green-50 p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fadeIn">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900">Teste do épico concluído e laudo arquivado.</p>
                      <p className="text-xs text-gray-500">
                        A evidência foi comprimida em GZIP e expurgada do bucket temporário.
                      </p>
                    </div>
                  </div>
