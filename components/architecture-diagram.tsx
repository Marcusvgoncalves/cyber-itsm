...
export function ArchitectureDiagram() {
  ...
  > = {
    ...
    backend: {
      title: "Edge Proxy, Conectores B2B & API",
      subtitle: "Roteamento Edge, Conectores MTLS & Rate Limit",
      desc: "Serverless API na Vercel com Middleware Proxy (`proxy.ts`). Executa verificações de autorização RBAC, previne estouro de quota, gerencia conectores externos B2B (Jira, ServiceNow, M365) exigindo Client Certificates (MTLS) e expõe endpoints de governança IAM.",
      details: [
        "Integrações nativas B2B seguras com Jira, ServiceNow e Microsoft 365",
        "Obrigatoriedade de certificados clientes via MTLS (Mutual TLS)",
        "Interceptação resiliente de Rate Limit (HTTP 429) e validação síncrona",
    ...
    ai: {
      ...
      desc: "Motor de inteligência artificial generativa com roteamento automatizado em cascata: 1) Groq (Llama 3.3 70B com Zod) -> 2) OpenRouter (Gemini & Llama Free) -> 3) Google Gemini (2.0 Flash) -> 4) Motor Determinístico de Fallback. Integra RAG sobre os 314 requisitos normativos e mantém persona rigorosa de SecOps com higiene de contexto.",
      details: [
      ...
      tools: ["Groq (Llama 3.3)", "OpenRouter Free LLMs", "Google Gemini 2.0", "Zero Memory Leak", "Zod", "RAG Engine"],
    ...
    security_qa: {
      ...
      desc: "Bounded Context isolado para auditoria autônoma de evidências de segurança e anexos (JSON, XML, TXT, DOCX, PDF, Imagens). Possui OCR e parsing avançado, compressão GZIP forense de artefatos brutos (< 10MB), expurgo (Zero Data Leak) e exportação robusta de PDF.",
      details: [
      ...
      tools: ["OCR / Document Parser", "PDF Native Export", "zlib/GZIP Forensics", "QA Engine API", "Recharts"],
